...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GenerarIncidencias` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `generar_asistencias_diarias` (OUT `mensaje` VARCHAR(255))  BEGIN
    ...
    IF es_no_lectivo = 0 THEN
        ...
        SET mensaje = CONCAT('Generadas ', conteo, ' asistencias para el día ', dia_actual);
    ELSE
        -- Guardar mensaje en la variable de salida
        SET mensaje = 'Hoy es un día no lectivo, no se generan asistencias.';
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `registrar_asistencia` (IN `p_identificador_profesor` VARCHAR(50), IN `p_numero_aula` INT)  proc_label: BEGIN  /* Añadimos la etiqueta aquí */
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `get_dia_espanol` () RETURNS VARCHAR(20) CHARSET utf8mb4 BEGIN
...
(29, 26, 1, 'Inteligencia Artificial Avanzada', '5ºA');
...
(53, 29, 'Miércoles', '08:00:00', '21:00:00'),
(56, 11, 'Jueves', '09:00:00', '21:00:00'),
(57, 11, 'Miércoles', '12:00:00', '14:00:00');
...
(8, 6, 1, 'Médico', '2025-03-14 11:56:22'),
...
(32, 30, 1, 'Aajsdna', '2025-04-01 12:35:00'),
...
(58, 58, 0, 'Prueba', '2025-05-12 18:31:31'),
(59, 59, 0, 'Prueba', '2025-05-12 18:31:31'),
...
CREATE TABLE `nodos` (
  ...
  `bateria` tinyint(3) UNSIGNED NOT NULL DEFAULT 100
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `nodos` (`id`, `aula_id`, `numero_aula`, `ultima_conexion`, `bateria`) VALUES
(1, 1, '0101', '2026-04-18 08:45:00', 87),
(2, 2, '0102', '2026-04-18 09:10:00', 62),
(3, 3, '0103', '2026-04-18 07:30:00', 15),
(4, 4, '0104', '2026-04-17 21:00:00', 5),
(5, 5, '0105', '2026-04-18 09:00:00', 95),
(6, 6, '0106', '2026-04-18 08:55:00', 73),
(7, 7, '0201', '2026-04-18 08:20:00', 40),
(8, 8, '0202', '2026-04-16 14:00:00', 22),
(9, 9, '0203', '2026-04-18 09:05:00', 100),
(10, 10, '0204', '2026-04-18 08:50:00', 55),
(11, 11, '0205', '2026-04-15 10:00:00', 8),
(12, 12, '0301', '2026-04-18 09:15:00', 80),
(13, 13, '0302', '2026-04-17 18:30:00', 33),
(14, 15, '4041', '2026-04-18 08:00:00', 68);
...
(26, 'María', 'González Rodríguez', '12345678X', 'maria.gonzalez@complutense.es', 1);
...
CREATE TABLE `usuarios` (
  ...
  `rol` enum('admin','editor','lector','profesor') NOT NULL DEFAULT 'lector',
...
INSERT INTO `usuarios` (`id`, `username`, `password`, `nombre`, `rol`, `IdProfesor`, `fecha_creacion`) VALUES
(5, 'admin', '$2y$10$Slf/MTCSdKc7NV96IJUw5uDGcYMcFpFydb4hDLtWET96ntAEmRqn2', 'Administrador', 'admin', NULL, '2025-04-30 09:25:56'),
(6, 'angel', '$2y$10$xDFL61OGKSefao.x0uJs7uhnEIlLLuvCg/T3PO.jirB0lCwxMur1W', 'Angel', 'profesor', '00000004D', '2026-01-18 12:44:02');
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `GeneradorAsistencias` ON SCHEDULE EVERY 1 DAY STARTS '2025-05-12 08:00:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
        IF WEEKDAY(CURRENT_DATE) BETWEEN 0 AND 4 THEN
            CALL generar_asistencias_diarias();
        END IF;
    END$$

CREATE DEFINER=`root`@`localhost` EVENT `GeneradorIncidencias` ON SCHEDULE EVERY 1 DAY STARTS '2025-05-12 21:00:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
        IF WEEKDAY(CURRENT_DATE) BETWEEN 0 AND 4 THEN
