...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            /* "Estado" como cabecera visual */
            .incidencias-table td[data-label="Estado"]{
                padding-top: 14px;
                padding-bottom: 14px;
                font-size: 16px;
                font-weight: 800;
            }
            .incidencias-table td[data-label="Estado"]::before{
                content: "Estado";
            }

            /* Acciones en fila y botones grandes */
            .incidencias-table td[data-label="Acciones"]{
        ...
        <div class="table-container">
            <table class="incidencias-table">
                ...
                <tbody>
                    ...
                    // Si es profesor, obtenemos su id real y filtramos para que solo vea sus incidencias
                    $idProfesorReal = null;
                    if ($_SESSION['rol'] === 'profesor' && !empty($_SESSION['IdProfesor'])) {
                        ...
                        if ($row_prof = $res_prof->fetch_assoc()) {
                            $idProfesorReal = (int)$row_prof['id'];
                        }
                        $stmt_prof->close();
                    }
                    ...
                    // Restricción por rol: el profesor solo ve sus incidencias
                    if ($_SESSION['rol'] === 'profesor' && !empty($idProfesorReal)) {
                        $conditions[] = "p.id = ?";
                        $params[] = $idProfesorReal;
                        $types .= "i";
                    }
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td data-label='Estado'>" .
                                ($row['justificada']
                                    ? '<span style="color:green;">✓ Justificada</span>'
                                    : '<span style="color:red;">✗ No justificada</span>') .
                                "</td>";

                            echo "<td data-label='Fecha'>" . htmlspecialchars($row['fecha_formateada']) . "</td>";
                            echo "<td data-label='Asignatura'>" . htmlspecialchars($row['nombre_asignatura']) . "</td>";
                            echo "<td data-label='Profesor'>" . htmlspecialchars($row['profesor']) . "</td>";
                            echo "<td data-label='Correo del Profesor'>" . htmlspecialchars($row['correo_profesor']) . "</td>";
                            echo "<td data-label='Acciones'><div class='action-buttons'>";
                            
                            // Diferentes acciones según el estado de la incidencia
                            if (!$row['justificada']) {
                                // Para incidencias no justificadas y si el usuario tiene permisos
                                $puedeJustificar = in_array($_SESSION['rol'], ['admin', 'editor'])
                                    || ($_SESSION['rol'] === 'profesor' && !empty($idProfesorReal) && (int)$row['profesor_id'] === (int)$idProfesorReal);
