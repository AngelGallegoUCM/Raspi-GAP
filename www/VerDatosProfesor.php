...
if ($result_profesor->num_rows > 0) {
    ...
    // Si es profesor, verificar que solo pueda ver sus propios datos
    if ($_SESSION['rol'] === 'profesor') {
        if (!isset($_SESSION['IdProfesor']) || $profesor['identificador'] !== $_SESSION['IdProfesor']) {
            header("Location: acceso_denegado.php");
            exit();
        }
    }
...
if ($result_horarios && $result_horarios->num_rows > 0) {
    while ($row = $result_horarios->fetch_assoc()) {
        ...
        if ($hora_fin_obj->format('i') > 0) $hora_fin_num++;
            ...
            if (strtoupper($row['nombre_asignatura']) == 'FAL' || strtoupper($row['nombre_asignatura']) == 'SO') {
                ...
                if (count($nombre_parts) > 1) {
                    ...
                    foreach ($nombre_parts as $part) {
                        if (!empty($part)) $siglas .= strtoupper(substr($part, 0, 1));
                    }
                } else {
...
<head>
    ...
    <style>
        ...
        .info-btn:hover { background-color: #2e59d9; }
        .info-btn.active {
        ...
        .volver    { background-color: #6c757d; }
        ...
        .identificador-group input { flex: 1; }
        .nfc-btn {
            padding: 10px 20px;
        ...
        .nfc-btn:hover     { background-color: #ff8533; }
        .nfc-btn:disabled  { background-color: #cccccc; cursor: not-allowed; }
        .nfc-btn .loading {
            display: inline-block;
            width: 16px;
            height: 16px;
        ...
        .nfc-modal-content h3 { margin-top: 0; color: #333; }
        .nfc-modal-content p  { margin: 15px 0; color: #666; }
        .nfc-modal-content .nfc-icon { font-size: 48px; margin-bottom: 15px; }
        ...
        .nfc-modal-content button:hover { background-color: #2e59d9; }
        .nfc-status {
            padding: 10px;
            margin: 10px 0;
            border-radius: 4px;
        ...
        .nfc-status.success { background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; }
        .nfc-status.error   { background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; }
        .nfc-status.info    { background-color: #d1ecf1; color: #0c5460; border: 1px solid #bee5eb; }

        @media (max-width: 770px) {
            .horario-table { font-size: 11px; }
            .horario-table td { padding: 4px; }
            .horario-table td.asignatura { font-size: 10px; padding: 2px; }
            .identificador-group { flex-direction: column; }
            .identificador-group input, .nfc-btn { width: 100%; }
    ...
    <script>
        ...
        function checkNFCSupport() {
            if (!('NDEFReader' in window)) {
                return { supported: false, message: 'Tu navegador no soporta Web NFC. Necesitas Chrome en Android.' };
            }
            return { supported: true };
        }

        function showModal() { nfcModal.classList.add('show'); }
        function hideModal() { nfcModal.classList.remove('show'); }
        ...
        if (writeNfcBtn) {
            writeNfcBtn.addEventListener('click', async () => {
                ...
                if (!nfcCheck.supported) { alert(nfcCheck.message); return; }

                const identificador = document.getElementById('identificador').value.trim();
                if (!identificador) { alert('No hay identificador para escribir'); return; }
                ...
                try {
                    ...
                    await ndef.write({ records: [{ recordType: "text", data: identificador }] });
                    updateModalStatus('', 'Escritura Exitosa', `Se escribió correctamente: "${identificador}"`, 'success');
                    closeModal.style.display = 'block';
                } catch (error) {
                    let errorMessage = 'Error desconocido';
                    if (error.name === 'NotAllowedError') errorMessage = 'Permiso denegado. Activa NFC en tu dispositivo.';
                    else if (error.name === 'NetworkError') errorMessage = 'No se detectó ninguna tarjeta NFC. Intenta de nuevo.';
        ...
        nfcModal.addEventListener('click', (e) => { if (e.target === nfcModal) hideModal(); });
    </script>
</body>
</html>